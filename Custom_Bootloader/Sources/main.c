...
#define APP_SIZE_LOCATION (0x9C04u)

/*Flash location that stores header of the file*/
#define APP_HEADER_LOCATION (0x9C08u)
...
static void Erase_Old_Application(void)
{
    ...
    if (app_start_addr == 0xFFFFFFFF && app_size_sector == 0xFFFFFFFF)
    {
        /*Do nothing*/
    }
    else
...
uint32_t Boot_main(void)
{
    ...
    {
        /*Get queue flag*/
        queue_flag = Driver_UART0_check_Rx_buffer();
...
int main(void)
{
    ...
    if (Driver_GPIO_read_pin_state(switch2.port_type, switch2.pin) == 1)
    {
        ...
        if (0XFFFFFFFF == Read_FlashAddress(APP_START_ADDRESS_LOCATION))
        {
        ...
        {
            while (Read_Flash_byte(APP_HEADER_LOCATION + i) != 0xFFu)
            {
                header[i] = Read_Flash_byte(APP_HEADER_LOCATION + i);
                i++;
            }
            Driver_UART0_send_string("\nApp    : ");
